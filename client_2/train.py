# client_2/train.py (Fast + Accurate Version)
...
CLIENT_CLASSES = ["walking", "marching", "biking"]

EPOCHS = 3
BATCH_SIZE = 6           # ✅ Larger batch → faster training
LR = 5e-4
...
class ClipDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # ✅ Use half frames (8 instead of 16)
        clip = clip[::2][:8]
...
def train():
    ...
    if len(ds) == 0:
        print("❌ No clips found. Please preprocess dataset first.")
        return
    ...
    print(f"[{CLIENT_NAME}] 🔐 Encryption Hash → {pkg['hash']}\n")
